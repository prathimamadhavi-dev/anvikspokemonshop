...
const TEMPLATE_ID = "templateuawt115";
...
let currentMode = null; // 'trade' | 'buy'
...
function sendRequest() {
  ...
  emailjs.send(EMAILJS_SERVICE_ID, TEMPLATE_ID, {
    ...
    time:       inputVal,   // "time" field holds the offer text per your template
  })
  .then(() => {
    document.getElementById('formArea').classList.remove('open');
  ...
  .catch(err => {
    console.error(err);
    showToast('Failed to send — check console for details', true);
  })
  .finally(() => {
    btn.disabled    = false;
...
function submitMeeting() {
  ...
  emailjs.send(EMAILJS_SERVICE_ID, TEMPLATE_ID, {
    ...
    type:       'MEETING',
    time:       dateVal + ' @ ' + locationVal + (noteVal ? ' — ' + noteVal : ''),
  })
  .then(() => {
